...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initMetadata() {
    metadata = ECGReport(
        id: Common.timestampNow(),
        dataInfo: '',
        label: LabelProvider.labels[0],
        name: '',
        age: -1,
        pediatrics: -1);
    ...
    getDatabasesPath().then((value) {
      ...
      openDatabase(databasePath, version: 1,
  ...
  bool validate() {
    if (!formKey.currentState!.validate()) {
      if (nameController.text.isEmpty) {
        ...
        return false;
      } else if (LabelProvider.selectedLabel! > 1 &&
          pediatricsController.text.isEmpty) {
  ...
  String _getLabel() {
    ...
    debugPrint(
        'Gender all outputs: ${_genderOutputs.map((e) => e.value).join(',')}');
    debugPrint(
        'Pregnancy all outputs: ${_pregnancyOutputs.map((e) => e.value).join(',')}');
    ...
    if (id == 0) {
      ...
      int g0 =
          _genderOutputs.where((e) => e.value < _models[0].threshold).length;
      int g1 = _genderOutputs.length - g0;

      if (g0 > g1) {
        int p0 = _pregnancyOutputs
            .where((e) => e.value < _models[1].threshold)
            .length;
        int p1 = _pregnancyOutputs.length - p0;

        if (p0 > p1) {
          return '${_models[0].labels[0]} ($g0/${_genderOutputs.length}), ${_models[1].labels[0]} ($p0/${_pregnancyOutputs.length})\nin $_averageTime ms';
        } else {
          return '${_models[0].labels[0]} ($g0/${_genderOutputs.length}), ${_models[1].labels[1]} ($p1/${_pregnancyOutputs.length})\nin $_averageTime ms';
        }
      } else {
        return '${_models[0].labels[1]} ($g1/${_genderOutputs.length}), ${_models[1].labels[0]}\nin $_averageTime ms';
      }
    }
  }
  ...
  void readVoltages() {
    ...
    var jsonArray = _selectedDataPoint!.value.toJson()['voltageValues']
        as List<Map<String, dynamic>>;
    var times = Iterable<int>.generate(jsonArray.length).toList();
    _voltages = jsonArray.map((e) {
  ...
  void _changedSelectedDataPoint(HealthDataPoint? selectedDataPoint) {
    setState(() {
      ...
      database
          .rawQuery(
              'SELECT * FROM $tableName WHERE id = ${metadata.id} LIMIT 1')
          .then((value) {
        ...
        if (value.isNotEmpty) {
          ...
          changedSelectedLabel(
              LabelProvider.labels.indexOf(value.first['label'].toString()));
        } else {
    ...
    try {
      ...
      client!.uploadMetadata(
        ...
        onTimeout: () {
          setState(() {
            _status = 'Connection failed';
            Common.showAlertDialog(this.context, 'Upload ECG Metadata',
                'Connection Timeout', const Duration(seconds: 2), Colors.red);
          });
        },
      );
    } catch (e) {
      setState(() {
        _status = 'Connection failed';
        Common.showAlertDialog(this.context, 'Upload ECG Metadata', 'Failed',
            const Duration(seconds: 2), Colors.red);
      });
      return;
    }
  }
